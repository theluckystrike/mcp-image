...
test.skip("watermark: pixels change where the text is drawn, and the source does not", async () => {
...
test.skip("a decompression bomb is refused from its header, before anything decodes it", async () => {
...
test.skip("an output that is one of the inputs is refused, by path and by symlink", async () => {
...
test.skip("dominant colours read the image and are Pro only", async () => {
