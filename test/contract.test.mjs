...
test.skip("stdout carries JSON-RPC only, including through a decoder that warns", async (t) => {
  const b = box();
  const c = client({ dataHome: b.dataHome });
  t.after(() => { c.close(); clean(b.dir); });
  await c.init();
...
test.skip("corrupt register: quarantined byte-for-byte, and the image work still succeeds", async (t) => {
  const b = box();
...
test.skip("cap: an output over the free megapixel limit writes no file at all", async (t) => {
  const b = box();
  const c = client({ dataHome: b.dataHome });
  t.after(() => { c.close(); clean(b.dir); });
  await c.init();
